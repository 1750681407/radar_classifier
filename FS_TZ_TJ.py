...
# 设置文件路径和编码
num = 9
file_path = f'./result/PDW{num}分类.csv'
encoding = 'gbk'
output_file_path = f'./result/PDW{num}分类统计结果.csv'

# 读取 CSV 文件（没有表头），并手动指定列名
columns = ['频率(Hz)', '到达时间(s)', '脉冲宽度(s)', '到达角度', '幅度', '航迹num']
data = pd.read_csv(file_path, encoding=encoding, header=None, names=columns)
