...
outputpath =  '.\result\';
outputfile = 'PDW15分类.csv';
trace_num = 6;

imagespath = '.\images\';

x = importdata([inputpath,inputfile]);    %数据读取
idx = x(:,4)>60;
x = x(idx,:);
...
legend(lgd);
saveas(gcf,[imagespath,outputfile,'_overview','.png']);
...
%     saveas(gcf,[imagespath,outputfile,num2str(p(1,10)),'.png']);
% end
%依次画出每类航迹
for ii = 1:length(n)
    iii = xx(:,10)==n(ii);
...
%  save('.\result\PDW8.mat')
% 指定输出文件名
% filename = '.\result\PDW8分类.csv';
